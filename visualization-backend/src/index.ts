...
const snapshotStore = new SnapshotStore(process.env.CLUSTER_NAME || "metalmart");
...
const operatorStatusPaths = [
  "/operator-status",
  "/visualization/api/operator-status",
];
...
const knownDeployments: KnownDeployment[] = [
  ...
  },
  {
    id: "order",
    name: "order",
    description: "Order API & Kafka producer",
    deployment: "order",
  },
  {
    id: "order-processor",
    name: "order-processor",
    description: "Kafka consumer & order status",
...
const defaultNamespace = process.env.WATCH_NAMESPACE || "metalmart";
...
const staticDbConnections: Record<string, string | undefined> = {
  "postgres": process.env.PG_ORDERS_URL ?? process.env.DATABASE_URL,
  "postgres-catalogue": process.env.PG_CATALOGUE_URL,
...
const dbTablesPaths = ["/db/:dbId/tables", "/visualization/api/db/:dbId/tables"];
const dbTableDataPaths = ["/db/:dbId/tables/:tableName", "/visualization/api/db/:dbId/tables/:tableName"];
