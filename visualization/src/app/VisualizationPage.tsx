...
export default function VisualizationPage({ useQueueSplittingMock, useDbBranchMock, useMultipleSessionMock }: VisualizationPageProps) {
  ...
  const agentGroups = useMemo((): AgentGroup[] => {
    const shopSessions = operatorSessions.filter(
      (s) => s.namespace === "metalmart" || s.namespace === "shop",
    );
  ...
  // Note: order-processor agent node comes from OperatorSession, not KafkaEphemeralTopic.
  const kafkaSplitTargets = useMemo(() => {
    ...
    for (const topic of kafkaTopics) {
      ...
      if (session && session.target.name !== "order-processor") {
  ...
  const kafkaTopicNodes = useMemo((): Node<NodeData>[] => {
    ...
    // 3. "Temporary Topic for Deployed App" node – positioned to the right of order-processor
    const deployedNodeId = "kafka-deployed-topic-delivery";
    const deliveryPos = adjustedNodes.find((n) => n.id === "order-processor")?.position ?? { x: 0, y: 0 };
    nodes.push({
      id: deployedNodeId,
      type: "mirrord",
      data: {
  ...
  const kafkaTopicEdges = useMemo((): Edge[] => {
    ...
    // Temporary Topic for Deployed App → order-processor (right to left)
    edges.push({
  ...
  // e.g. if a split topic sits between "kafka" and "order-processor", hide "kafka-to-order-processor".
